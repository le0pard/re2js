...
export class CharClass {
 ...
 if (srcPts !== dstPts || dstOff >= srcOff + size) {
    ...
    /*private*/ static cmp(array, i, pivotFrom, pivotTo) {
        const cmp = array[i] - pivotFrom
        return cmp !== 0 ? cmp : pivotTo - array[i + 1]
    }
    /*private*/ static qsortIntPair(array, left, right) {
        ...
        for (let i = 0; i < len; i += 2) {
            {
                ...
                const hi = r[i + 1]
                if (lo === hi) {
                    /* append */ (sb => { sb.str += '0x'; return sb })(b);
                    /* append */ (sb => { sb.str += Number(lo).toString(16); return sb })(b)
                } else {
                    /* append */ (sb => { sb.str += '0x'; return sb })(b);
                    /* append */ (sb => { sb.str += Number(lo).toString(16); return sb })(b);
                    /* append */ (sb => { sb.str += '-0x'; return sb })(b);
                    /* append */ (sb => { sb.str += Number(hi).toString(16); return sb })(b)
                }
            }
        }
        /* append */ (sb => { sb.str += ']'; return sb })(b)
...
CharClass['__class'] = 'quickstart.CharClass'
