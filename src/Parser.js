...
export class Parser {
    constructor(wholeRegexp, flags) {
        ...
        this.stack = (() => { let __o = new Parser.Stack(); __o.__delegate = []; return __o })()
    ...
    pop() {
        return /* remove */ this.stack.__delegate.splice(/* size */ this.stack.__delegate.length - 1, 1)[0]
    }
    popToPseudo() {
        const n = this.stack.__delegate.length
        let i = n
        while ((i > 0 && !Regexp.Op['_$wrappers'][/* get */this.stack.__delegate[i - 1].op].isPseudo())) {
            {
                i--
            }
        }
 ...
 let a = []; while (s-- > 0) {a.push(null)} return a
})(n - i), /* subList */ this.stack.__delegate.slice(i, n))
    ...
    cleanAlt(re) {
        if (re.op === Regexp.Op.CHAR_CLASS) {
            ...
            } else if (re.runes.length === 4 && re.runes[0] === 0 && re.runes[1] === '\n'.charCodeAt(0) - 1 && re.runes[2] === '\n'.charCodeAt(0) + 1 && re.runes[3] === Unicode.MAX_RUNE) {
                re.runes = null
 ...
 if (srcPts !== dstPts || dstOff >= srcOff + size) {
                    ...
                    for (let j = start + 1; j < i; j++) {
                        {
                            ...
                            if (/* Enum.ordinal */Regexp.Op[Regexp.Op[subMax.op]] < /* Enum.ordinal */ Regexp.Op[Regexp.Op[subJ.op]] || (subMax.op === subJ.op && (subMax.runes != null ? subMax.runes.length : 0) < (subJ.runes != null ? subJ.runes.length : 0))) {
                                max = j
                            }
                        }
                    }
    ...
    parsePerlFlags(t) {
        ...
        const s = t.rest()
        if (/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(s, '(?P<')) {
            const end = s.indexOf('>')
            if (end < 0) {
        ...
        for (let i = 0; i < name.length; ++i) {
            {
                ...
                if ((c => c.charCodeAt == null ? c : c.charCodeAt(0))(c) != '_'.charCodeAt(0) && !Utils.isalnum((c).charCodeAt(0))) {
                    return false
                }
            }
        }
    ...
    static parseInt(t) {
        ...
        if (/* isEmpty */(n.length === 0) || (n.length > 1 && (c => c.charCodeAt == null ? c : c.charCodeAt(0))(n.charAt(0)) == '0'.charCodeAt(0))) {
            return -1
        }
        if (n.length > 8) {
            return -2
    ...
    swapVerticalBar() {
        const n = this.stack.__delegate.length
        if (n >= 3 && /* get */ this.stack.__delegate[n - 2].op === Regexp.Op.VERTICAL_BAR && Parser.isCharClass(/* get */ this.stack.__delegate[n - 1]) && Parser.isCharClass(/* get */ this.stack.__delegate[n - 3])) {
            let re1 = this.stack.__delegate[n - 1]
            let re3 = this.stack.__delegate[n - 3]
            if (/* Enum.ordinal */Regexp.Op[Regexp.Op[re1.op]] > /* Enum.ordinal */ Regexp.Op[Regexp.Op[re3.op]]) {
                const tmp = re3
                re3 = re1
                re1 = tmp;
    ...
    parsePerlClassEscape(t, cc) {
        ...
        if ((this.flags & RE2Flags.PERL_X) === 0 || !t.more() || t.pop() != '\\'.charCodeAt(0) || !t.more()) {
            return false
        }
        t.pop()
        const g = ((m, k) => m[k] === undefined ? null : m[k])(CharGroup.PERL_GROUPS_$LI$(), t.from(beforePos))
        if (g == null) {
            return false
        }
    ...
    parseNamedClass(t, cc) {
        ...
        t.skipString(name)
        const g = ((m, k) => m[k] === undefined ? null : m[k])(CharGroup.POSIX_GROUPS_$LI$(), name)
        if (g == null) {
 ...
 if (Parser.ANY_TABLE == null) {
    ...
    parseUnicodeClass(t, cc) {
        const startPos = t.pos()
        if ((this.flags & RE2Flags.UNICODE_GROUPS) === 0 || (!t.lookingAt$java_lang_String('\\p') && !t.lookingAt$java_lang_String('\\P'))) {
            return false
        }
        t.skip(1)
        let sign = +1
        ...
        if (!(name.length === 0) && (c => c.charCodeAt == null ? c : c.charCodeAt(0))(name.charAt(0)) == '^'.charCodeAt(0)) {
            sign = -sign
        ...
        while ((!t.more() || t.peek() != ']'.charCodeAt(0) || first)) {
            {
                if (t.more() && t.lookingAt$char('-') && (this.flags & RE2Flags.PERL_X) === 0 && !first) {
                    const s = t.rest()
                    if ((s === ('-')) || !((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(s, '-]')) {
                        t.rewindTo(startPos)
                        throw new PatternSyntaxException(Parser.ERR_INVALID_CHAR_RANGE, t.rest())
                    }
                }
...
(function(Parser) {
    ...
    class StringIterator {
        ...
        lookingAt$char(c) {
            return (c => c.charCodeAt == null ? c : c.charCodeAt(0))(this.str.charAt(this.__pos)) == (c => c.charCodeAt == null ? c : c.charCodeAt(0))(c)
        }
        lookingAt$java_lang_String(s) {
            return /* startsWith */ ((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(this.rest(), s)
        }
        lookingAt(s) {
